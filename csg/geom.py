...
sys.setrecursionlimit(1000)  # my default is 1000, increasing too much may cause a seg fault
...
class Vertex(object):
    ...
    colors. Custom vertex classes need to provide a `pos` property and
    `clone()`, `flip()`, and `interpolate()` methods that behave analogous to
    the ones defined by `Vertex`. This class provides `normal` so convenience
    functions like `CSG.sphere()` can return a smooth vertex normal, but
...
class Plane(object):
    ...
    def splitPolygon(self, polygon, coplanarFront, coplanarBack, front, back):
        ...
        respect to this plane. Polygons in front or in back of this plane go
        into either `front` or `back`
        """
...
class Polygon(object):
    ...
    def __init__(self, vertices, shared=None):
        ...
        self.shared = shared
        self.plane = Plane.fromPoints(vertices[0].pos,
                                      vertices[1].pos,
                                      vertices[2].pos)
    ...
    def __repr__(self):
        return '[{}{}]'.format((
            repr(self.vertices[0]),
            [', {}'.format(repr(v)) for v in self.vertices[1:]]))
        # return reduce(lambda x, y: x+y,
        #     ['['] + [repr(v) + ', ' for v in self.vertices] + [']'], '')
...
class BSPNode(object):
    ...
    def build(self, polygons):
        ...
        back = []
        for poly in polygons:
            self.plane.splitPolygon(poly, self.polygons, self.polygons, front,
                                    back)
        if len(front):
            if not self.front:
                self.front = BSPNode()
